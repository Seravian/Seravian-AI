...
def generate_speech(request_dict):
    ...
    # Generate speech
    try:
        logger.info(f"Generating speech for text: '{request['text'][:50]}...' with preset: {preset}")
...
async def tts(request: TTSRequest,api_key:str = Depends(get_api_key)) -> TTSResponse:
    ...
    try:
        ...
        logger.info(f"Received TTS request: voice={request.voice}, preset={request.preset}")
...
@app.function(image=image, cpu=1.0, volumes={"/model_cache": model_cache_volume, "/logs": log_volume})
@modal.asgi_app()
